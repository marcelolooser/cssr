...
class Frames:
    ...
    def heaviside_overcomplete(self, box_width_interval=None, step_size=None):
        ...
        a_frame = np.zeros((self.signal_length, (delta//step_size)*self.signal_length - corr), dtype=complex) # shape[1] = n(n+1)/2 - 1, spark = 3
        ...
        for j in range(a_frame.shape[1]):
            ...
            a_frame[index, j] = 1/np.sqrt(box_width)
    ...
    def gaussian_overcomplete(self,  sigma_interval=None, step_size=None):
        ...
        a_frame = np.zeros((self.signal_length, self.signal_length*n), dtype=complex)
        for j, sigma in enumerate(py.arange(sigma_interval[0], sigma_interval[1], step_size)):
            for i, mu in enumerate(self.x):
                gdist = scipy.stats.norm.pdf(self.x, mu, sigma)
                a_frame[:,j*self.signal_length + i] = gdist/np.linalg.norm(gdist, 2)
        return a_frame
    ...
    def cauchy_overcomplete(self, gamma_interval=None, step_size=None):
        ...
        for j, gamma in enumerate(py.arange(gamma_interval[0], gamma_interval[1], step_size)):
            for i, mu in enumerate(self.x):
                ...
                a_frame[:,j*self.signal_length + i] = cdist/np.linalg.norm(cdist, 2)
        return a_frame
    ...
    def _wavelet(self, *args):
        """Wavelet dictionary."""
        raise NotImplementedError("Wavelet frames are not yet implemented.")


    # implementation needed
    def _mega_dict(self, *args):
        """Combines all frames for a mega dictionary."""
        raise NotImplementedError("Mega dictionaries are not yet implemented.")


    # implementation needed
    def _mega_dict_overcomplete(self, *args):
        """Combines all frames for a mega overcomplete dictionary."""
        raise NotImplementedError("Mega overcomplete dictionaries are not yet implemented.")
